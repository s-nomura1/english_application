...
if "messages" not in st.session_state:
    ...
    # 提出課題 Start 英語レベルの初期値設定
    st.session_state.englv = "初級者"
    # 提出課題 End 英語レベルの初期値設定
...
if st.session_state.start_flg:
    ...
    # 提出課題 Start 英語レベルの説明取得
    # 英語レベルの説明を取得（英語プロンプト用）
    english_level_desc = ct.ENGLISH_LEVEL_DESCRIPTION.get(st.session_state.englv, "intermediate level")
    # 英語レベルの説明を取得（日本語プロンプト用）
    english_level_desc_jp = ct.ENGLISH_LEVEL_DESCRIPTION_JP.get(st.session_state.englv, "初心者（基本的な単語、基礎文法、短い文章）")
    # 提出課題 End 英語レベルの説明取得
    ...
    if st.session_state.mode == ct.MODE_3 and (st.session_state.dictation_button_flg or st.session_state.dictation_count == 0 or st.session_state.dictation_chat_message):
        if st.session_state.dictation_first_flg:
            ...
            # 英語レベルの説明を取得してテンプレートに反映
            system_template_problem = ct.SYSTEM_TEMPLATE_CREATE_PROBLEM.format(english_level=english_level_desc)
            st.session_state.chain_create_problem = ft.create_chain(system_template_problem)
        ...
        # チャット入力時の処理
            ...
            with st.spinner('評価結果の生成中...'):
                system_template = ct.SYSTEM_TEMPLATE_EVALUATION.format(
                    # 提出課題 Start プロンプトに英語レベルを追加
                    english_level=english_level_desc_jp,
                    # 提出課題 End プロンプトに英語レベルを追加
    ...
    if st.session_state.mode == ct.MODE_1:
        ...
        # 提出課題 Start プロンプトに英語レベルを追加
        # 英語レベルの説明を取得してテンプレートに反映
        system_template_conversation = ct.SYSTEM_TEMPLATE_BASIC_CONVERSATION.format(english_level=english_level_desc)
        st.session_state.chain_basic_conversation = ft.create_chain(system_template_conversation)
        # 提出課題 End プロンプトに英語レベルを追加
